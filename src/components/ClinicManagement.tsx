...
import { Search, Plus, Edit, Trash2, Eye, Phone, MapPin, Calendar, QrCode, Download, Database, Filter, X, Upload } from 'lucide-react';
import {
  Table,
  TableBody,
  TableCell,
...
const ClinicManagement: React.FC = () => {
  ...
  const filteredClinics = clinics.filter(clinic => {
    ...
    const matchesGovernorate = governorateFilter === 'all' || clinic.governorate === governorates.find(g => g.id === governorateFilter)?.name;
    const matchesCity = cityFilter === 'all' || clinic.city === cities.find(c => c.id === cityFilter)?.name;
    
    return matchesSearch && matchesStatus && matchesSpecialization && matchesGovernorate && matchesCity;
  });
  ...
  const hasActiveFilters = statusFilter !== 'all' || specializationFilter !== 'all' || searchTerm !== '' || governorateFilter !== 'all' || cityFilter !== 'all';
  ...
  if (showDetails && selectedClinic) {
    ...
    const handlePrint = () => {
      const params = new URLSearchParams({
        ...
        license_number: selectedClinic.license_number || '',
        doctor_name: selectedClinic.doctor_name || '',
        specialization: selectedClinic.specialization || '',
        governorate: selectedClinic.governorate || '',
        city: selectedClinic.city || '',
        address: selectedClinic.address || '',
      }).toString();
      navigate(`/admin/print-qr?${params}`);
    };
    return (
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={handlePrint}>
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2m-6 0v4m0 0h4m-4 0H8" /></svg>
              طباعة
            </Button>
            ...
            <div className="col-span-2 space-y-8">
              ...
              <div>
                ...
                <div className="flex flex-col gap-2 text-right">
                  ...
                  <span className="text-lg flex items-center gap-1"><span className="font-bold">رقم الهاتف:</span> <span dir="ltr" style={{direction: 'ltr', unicodeBidi: 'isolate'}}>{selectedClinic.phone}</span></span>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* مربع معلومات الترخيص */}
                <div className="bg-gray-50 p-6 rounded-lg space-y-3 border">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-base" dir="rtl">
                  {selectedClinic.issue_date && (
                      <>
                        <span className="font-bold text-right">تاريخ الإصدار:</span>
                        <span className="text-left">{new Date(selectedClinic.issue_date).toLocaleDateString('ar-JO')}</span>
                      </>
                  )}
                  {selectedClinic.expiry_date && (
                      <>
                        <span className="font-bold text-right">تاريخ الانتهاء:</span>
                        <span className="text-left">{new Date(selectedClinic.expiry_date).toLocaleDateString('ar-JO')}</span>
                      </>
                  )}
                    <span className="font-bold text-right">عدد مرات التحقق:</span>
                    <span className="text-left">{selectedClinic.verification_count}</span>
                  </div>
                </div>
                {/* مربع العنوان */}
                <div className="bg-gray-50 p-6 rounded-lg border">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-base" dir="rtl">
                    <span className="font-bold text-right">المحافظة:</span>
                    <span className="text-left">{selectedClinic.governorate || <span className="text-gray-400">غير محددة</span>}</span>
                    <span className="font-bold text-right">المدينة:</span>
                    <span className="text-left">{selectedClinic.city || <span className="text-gray-400">غير محددة</span>}</span>
                    <span className="font-bold text-right">تفاصيل العنوان:</span>
                    <span className="text-left">{selectedClinic.address_details || <span className="text-gray-400">غير متوفر</span>}</span>
                    <span className="font-bold text-right border-t pt-2 col-span-1">العنوان الكامل:</span>
                    <span className="text-left border-t pt-2 col-span-1">{selectedClinic.address || <span className="text-gray-400">غير متوفر</span>}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              <Select value={specializationFilter} onValueChange={setSpecializationFilter}>
                <SelectTrigger className="w-40"><SelectValue placeholder="التخصص" /></SelectTrigger>
                <SelectContent>
              ...
              <Select value={governorateFilter} onValueChange={v => { setGovernorateFilter(v); setCityFilter('all'); }}>
              ...
              <Select value={cityFilter} onValueChange={setCityFilter} disabled={filteredCities.length === 0}>
              ...
              <Button onClick={() => exportToCSV(filteredClinics)} className="gap-2" disabled={filteredClinics.length === 0}>
          ...
          <div className="rounded-md border overflow-x-auto">
            <Table className="min-w-full">
              <TableHeader>
                <TableRow>
                  <TableHead className="text-right break-words whitespace-pre-line max-w-xs">اسم العيادة</TableHead>
                  <TableHead className="text-right break-words whitespace-pre-line max-w-xs">رقم الترخيص</TableHead>
                  <TableHead className="text-right break-words whitespace-pre-line max-w-xs">الطبيب</TableHead>
                  <TableHead className="text-right break-words whitespace-pre-line max-w-xs">التخصص</TableHead>
                  <TableHead className="text-right break-words whitespace-pre-line max-w-xs">الحالة</TableHead>
                  <TableHead className="text-right break-words whitespace-pre-line max-w-xs">عدد التحقق</TableHead>
                  <TableHead className="text-right break-words whitespace-pre-line max-w-xs">العنوان الكامل</TableHead>
                  <TableHead className="text-right break-words whitespace-pre-line max-w-xs">الإجراءات</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {currentClinics.map((clinic) => (
                  <TableRow key={clinic.id}>
                    <TableCell className="font-medium text-right break-words whitespace-pre-line max-w-xs">{clinic.clinic_name}</TableCell>
                    <TableCell className="font-mono text-right break-words whitespace-pre-line max-w-xs">{clinic.license_number}</TableCell>
                    <TableCell className="text-right break-words whitespace-pre-line max-w-xs">{clinic.doctor_name || '-'}</TableCell>
                    <TableCell className="text-right break-words whitespace-pre-line max-w-xs">{clinic.specialization}</TableCell>
                    <TableCell className="text-right break-words whitespace-pre-line max-w-xs">{getStatusBadge(clinic.license_status)}</TableCell>
                    <TableCell className="text-right break-words whitespace-pre-line max-w-xs">{clinic.verification_count}</TableCell>
                    <TableCell className="text-right break-words whitespace-pre-line max-w-xs">{clinic.address}</TableCell>
                    <TableCell className="break-words whitespace-pre-line max-w-xs">
              ...
              <PaginationContent>
                ...
                {(() => {
                  ...
                  if (totalPages <= 7) {
                    ...
                    // Always show first 2, last 2, current, and 2 before/after current
