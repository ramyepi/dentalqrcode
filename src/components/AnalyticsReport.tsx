...
const AnalyticsReport: React.FC = () => {
  ...
  // توزيع المدن ديناميكياً حسب المحافظة المختارة
  const filteredCityData = useMemo(() => {
    ...
    return Object.entries(cityDist)
      ...
      .sort((a, b) => b.count - a.count);
  }, [selectedGov, clinics, governorates]);

  // [أضف أعلى الكومبوننت مباشرةً، بعد useState للمدن والمحافظات]
  const [selectedGovForSpec, setSelectedGovForSpec] = useState<string>('all');
  const [selectedCityForSpec, setSelectedCityForSpec] = useState<string>('all');

  // استخراج قائمة المدن المتاحة حسب المحافظة المختارة
  ...
  // توزيع التخصصات للعيادات المفلترة
  const filteredSpecializationData = useMemo(() => {
    const total = filteredClinicsForSpec.length;
    const dist: Record<string, number> = {};
    filteredClinicsForSpec.forEach(clinic => {
      const spec = clinic.specialization || 'غير محدد';
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip 
                    ...
                    labelFormatter={() => ''}
                    content={renderPieTooltip('name', 'value', 'percentage', 'الحالة')}
                  />
                </PieChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip 
                    content={renderPieTooltip('specialization', 'count', 'percentage', 'التخصص')}
                  />
                </PieChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip 
                    content={renderPieTooltip('name', 'count', 'percentage', 'المحافظة')}
                  />
                </PieChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="h-80">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip 
                    content={renderPieTooltip('name', 'count', 'percentage', 'المدينة')}
                  />
                </PieChart>
              </ResponsiveContainer>
              )}
      ...
      {/* --- Pie Chart: Specialization Distribution by Governorate & City --- */}
      <div className="mt-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Stethoscope className="h-5 w-5" />
              توزيع التخصصات حسب المحافظة والمدينة
            </CardTitle>
            <div className="mt-4 flex flex-wrap gap-4 items-center">
              <div className="flex items-center gap-2">
                ...
                <select
                  className="border rounded px-2 py-1"
                  value={selectedGovForSpec}
                  onChange={e => {
                    setSelectedGovForSpec(e.target.value);
                    setSelectedCityForSpec('all');
                  }}
                >
                  <option value="all">كل المحافظات</option>
                  {governorates.map(gov => (
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  className="border rounded px-2 py-1"
                  value={selectedCityForSpec}
                  onChange={e => setSelectedCityForSpec(e.target.value)}
                  disabled={filteredCitiesForSpec.length === 0}
                >
          ...
          <CardContent>
            <div className="h-80">
              {filteredSpecializationData.length === 0 ? (
                <div className="flex items-center justify-center h-full text-gray-400 text-lg">لا توجد بيانات لعرضها</div>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={filteredSpecializationData}
                      cx="50%"
                      cy="50%"
                      labelLine={false}
                      label={renderCustomizedLabel}
                      ...
                      dataKey="count"
                    >
                      {filteredSpecializationData.map((entry, index) => (
                        <Cell key={`cell-spec-filtered-${index}`} fill={COLORS[index % COLORS.length]} />
                      ))}
                    </Pie>
                    <Tooltip 
                      content={renderPieTooltip('specialization', 'count', 'percentage', 'التخصص')}
            ...
            {filteredSpecializationData.length > 0 && (
              <div className="mt-4 grid grid-cols-2 gap-2 max-h-40 overflow-y-auto">
                {filteredSpecializationData.map((item, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <div 
                      className="w-3 h-3 rounded-full" 
                      style={{ backgroundColor: COLORS[index % COLORS.length] }}
